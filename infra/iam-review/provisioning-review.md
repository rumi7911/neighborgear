# Provisioning review — 9 September 2026

**Local drafts only. Do not attach these policies or deploy either template yet.**

## Prepared artifacts

### Network ownership split

The local design now separates `infra/network-bootstrap.yaml` (owner-managed, proposed stack `neighborgear-network`) from `infra/template.yaml` (application stack `neighborgear-demo`). The owner network template contains the HTTP API and auto-deploying throttled default stage, CloudFront distribution/origin policies, private web bucket and its distribution-scoped policy. It is disabled by default and credit-gated. This is a preparation change only: neither stack has been created and no existing resource is being migrated.
...
This moves the unscopable network-creation operations out of the application deployer's remit; it does not make them narrowly scoped AWS operations. The owner must separately review and approve the exact network creation/update change set and establish the required owner-authorized, non-root execution path. Do not give the application role a broad network role or permission to operate the network stack. Its current CloudFormation stack scope remains only `neighborgear-demo`.
...
1. **API Gateway HTTP API:** the local ownership split and exact-ID route/integration policy are prepared. Verify actual IDs, IAM enforcement and CloudFormation create/update/rollback in AWS after approval. API and stage creation now belong to the owner network setup; no tag inheritance assumption is needed for app grants.
2. **CloudFront:** the local ownership split avoids granting these generated-ID creation/management operations to the app role. Owner network execution authority and the exact initial change set still require review. No application CloudFront grant has been added.
...
None of these reviews authorizes account-wide grants. AWS plugin reauthentication and a verified non-root deployment session remain necessary; public documentation checks did not refresh account evidence. Lambda quota, Scheduler credit coverage, usage-stop controls and cost estimates remain independent blockers.
