...
The [provisioning review](provisioning-review.md) now explains the named-resource policy component, all of its scope exceptions, and the remaining API Gateway, AgentCore and dependency gaps. `cloudformation-named-resources.json` covers named infrastructure configuration, not a complete deployment service role. `infra/security-bootstrap.yaml` is an owner-only, disabled scaffold: even when creation is enabled, both deployment roles retain explicit deny-all quarantine and no operator attachment is made. No executable unlock or application deployment path is claimed. The only deployed project resources are the separately approved network prerequisite.
