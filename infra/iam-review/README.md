...
[operator-preflight-readonly.json](operator-preflight-readonly.json) and its [scope review](operator-preflight-readonly.md) now mirror the seven approved non-mutating operations. This does not activate deployment access.
...
AgentCore follow-up: [the AgentCore review](agentcore-review.md) now accompanies `cloudformation-agentcore.json` and the offline `scripts/build_access_review.py` assembler. Tagged Ireland creation, runtime/endpoint management and exact-identity cleanup are drafted; Instances capacity-provider selection is explicitly denied. The critical live first-create/rollback identity dependency is not solved by placeholders. The assembler now produces a strict simulator subset without AgentCore/Bedrock or Runtime-role PassRole, while live assembly still refuses unresolved scopes and identities. Both modes remain review-only. The deployed roles remain quarantined; activation, live AgentCore creation/recovery and cloud validation are separate gates.

Network refinement: CloudFront, the HTTP API/default stage and web bucket live in the separately owned `neighborgear-network` stack, whose initial reviewed deployment completed on 12 September 2026. The app consumes its privately captured outputs and manages only routes/integrations inside that API through `cloudformation-api-routes.json`. Web-bucket administration has been removed from the app's named-resource component. This avoids an account-wide CloudFront grant to the app role. Exact deployed-API policy-editor validation and positive/negative API Gateway V2 simulation are complete; AgentCore provisioning remains pending. See the provisioning review for cross-stack setup and shutdown implications.

The [provisioning review](provisioning-review.md) now explains the named-resource policy component, all of its scope exceptions, and the remaining API Gateway, AgentCore and dependency gaps. `cloudformation-named-resources.json` covers named infrastructure configuration, not a complete deployment service role. The owner executed `infra/security-bootstrap.yaml` after its separate review and action-time approval: both deployed roles retain explicit deny-all quarantine and no operator attachment was made. No executable unlock or application deployment path is claimed. The deployed project resources are limited to the separately approved network prerequisite and quarantined security scaffold.
...
**Still unfinished:** the complete CloudFormation infrastructure-provisioning policy and activation of the owner-controlled bootstrap. The locked scaffold is now defined, but its final permission assembly is not complete. The work above does not make task 1 or AWS deployment complete. The exact deployed-API component has live editor and simulation evidence, but remaining AgentCore/workload-identity scopes still require live validation. No application deployment or model invocation was performed.
