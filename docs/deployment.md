...
Status: initial application implementation and offline validation complete; the separately approved network prerequisite is `CREATE_COMPLETE` in Ireland, while the **application itself is not deployed or tested end-to-end in AWS**. The ARM64 archive was built locally. Issued credits, core service coverage and Lambda quota compatibility were verified. The CloudFront endpoint currently has no frontend assets or application route. Non-root application deployment access, Scheduler coverage and live stop-control verification remain gates. See `credits-preflight.md`.
