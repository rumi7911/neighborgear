...
Security update, 12 September: after local lint, structural tests, selected AWS Guard rules, AWS deployment validation and a separate action-time owner approval, `infra/security-bootstrap.yaml` was executed as stack `neighborgear-security`. Its private artifact bucket, TLS policy, retained control table and two explicitly deny-all-quarantined roles all reached `CREATE_COMPLETE`. The roles still provide no deployment capability. The full service-role policy, workload boundary and controlled activation remain incomplete; do not apply the drafts independently or attempt application deployment with root. Lambda's applied quota is now verified at 1,000.
