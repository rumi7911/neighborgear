...
Security update, 13 September: after local lint, structural tests, selected AWS Guard rules, AWS deployment validation and separate action-time owner approvals, the security scaffold and workload boundary reached `CREATE_COMPLETE`. The simulator-only activation branch was uploaded as `neighborgear-simulator-access-review-20260913`; CloudFormation reports `CREATE_COMPLETE` / `AVAILABLE`, all supported validations passed, and the exact diff is two in-place role policy updates plus one MFA-gated operator policy. It remains unexecuted, so the deployed roles are still deny-all quarantined. Do not apply the policy components independently or attempt application deployment with root. Lambda's applied quota is verified at 1,000.
