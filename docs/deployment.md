...
Security update, 13 September: after local lint, structural tests, selected AWS Guard rules, AWS deployment validation and separate action-time owner approvals, the security scaffold and workload boundary reached `CREATE_COMPLETE`. The simulator-only activation `neighborgear-simulator-access-review-20260913` passed all supported validations and then executed successfully: the two IAM roles updated in place and the MFA-gated operator policy was created. The security stack is `UPDATE_COMPLETE`; application deployment and model invocation have not started. Do not apply the policy components independently or attempt application deployment with root. Lambda's applied quota is verified at 1,000.
